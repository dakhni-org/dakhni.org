...
- `write_sitemap()` in `scripts/build_site.py` computes each page's `<lastmod>` from `git log` on its source content file, not the current date — so `sitemap.xml` is only reproducible once the commit touching that content file actually exists in git history. If you edit a content file and run the build *before* committing, the baked-in `lastmod` for that file reflects the *previous* commit that touched it, not the one you're about to make; CI (which always builds from an already-committed checkout) will then compute a different date and fail the diff check on push. Any commit that edits a `content/**/*.json` file needs a rebuild done *after* that commit exists — commit the content change, rebuild, and if `sitemap.xml` changes again, add that as a follow-up commit before pushing.
